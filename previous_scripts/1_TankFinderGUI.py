...
#!/usr/bin/env python3
# TankFinder GUI (Tkinter) — hardened
...
def open_file_resilient(path: Path) -> None:
    """Try opening a file; fall back to Explorer select for long paths."""
    p = str(path)
    try:
...
class App(tk.Tk):
    def __init__(self):
        ...
        years_entry = ttk.Entry(top, textvariable=self.years_var, width=16)
        ...
        limit_entry = ttk.Entry(top, textvariable=self.limit_var, width=7)
        ...
        for c in self.job_cols:
            ...
            self.jobs.column(c, width={
                "job_id":90, "hits":70, "pdfs":60, "cad":60, "compress":90, "ame":60,
                "badges":180, "root_path":650
            }[c], anchor="w")
        ...
        self.file_filter = ttk.Combobox(header, textvariable=self.file_filter_var, width=12,
        ...
        # DB (read-only, friendly timeouts)
        if not DB_PATH.exists():
            messagebox.showerror("TankFinder", f"Database not found:\n{DB_PATH}")
            self.destroy(); return
        self.con = sqlite3.connect(
            f"file:{DB_PATH}?mode=ro&cache=shared", uri=True, timeout=5.0
        )
        self.con.row_factory = sqlite3.Row
        try:
            self.con.execute("PRAGMA query_only=ON")
            self.con.execute("PRAGMA busy_timeout=4000")
        except Exception:
            pass

        # detect if jobs has job_year (optional speed-up)
        try:
    ...
    def sort_tree(self, tv: ttk.Treeview, col: str):
        ...
        for c in columns:
            tv.heading(c, text=HEADINGS.get(c, c))
    ...
    def run_search(self):
        ...
        # years: prefer job_year if available; else fallback to LIKE
    ...
    def on_open_file(self, *_):
        ...
        try:
            if len(str(full)) >= 248:
                self.set_status("Very long path; Windows long-path policy recommended.")
    ...
    def open_sql_console(self):
        ...
        txt = tk.Text(win, wrap="none", height=8)
        txt.pack(side=tk.TOP, fill=tk.BOTH, expand=False, padx=8, pady=(8,4))
        txt.insert("1.0",
                   "SELECT COUNT(*) AS jobs FROM jobs;\n"
                   "SELECT COUNT(*) AS files FROM files WHERE deleted=0;\n"
                   "-- Jobs by year:\n"
                   "SELECT substr(root_path, instr(root_path,'\\JOBS\\')+6, 4) AS year, COUNT(*) AS jobs\n"
                   "FROM jobs GROUP BY year ORDER BY year;")

        res_frame = ttk.Frame(win); res_frame.pack(side=tk.BOTTOM, fill=tk.BOTH, expand=True, padx=8, pady=(0,8))
        tv = ttk.Treeview(res_frame, columns=("col1",), show="headings")
        vs = ttk.Scrollbar(res_frame, orient="vertical", command=tv.yview)
        ...
        def run_sql():
            ...
            try:
                ...
                if last_cols is not None:
                    ...
                    tv["columns"] = last_cols
                    for c in last_cols:
                        tv.heading(c, text=c); tv.column(c, width=max(120, int(900/len(last_cols))), anchor="w")
                    for r in last_rows:
    ...
    def refresh_index(self):
        ...
        def runner():
            try:
                ...
                code = proc.wait()
                if code == 0:
                    self.set_status("Index refresh complete")
                else:
                    self.set_status(f"Index refresh exited with code {code}")
            except Exception as e:
